...
데스크톱 버전은 트레이/메뉴 막대, 창을 닫은 뒤 백그라운드 실행,
로그인 자동 시작, 절전·네트워크 복구, 알림 딥 링크와 읽지 않은 배지를
지원합니다. **화면 설정 → 데스크톱 백그라운드**에서 구성할 수 있습니다.
자세한 내용은
[`docs/PHASE5_DESKTOP_LIFECYCLE.md`](docs/PHASE5_DESKTOP_LIFECYCLE.md)를
참조하세요.
